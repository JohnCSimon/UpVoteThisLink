...
type UVTLController() = 
    ...
    let getUpVotesForShortKey shortKey = 
        Query.all<UpVoteRecord>|> Query.where <@ fun g s -> s.PartitionKey = shortKey @> |> fromUpVoteTable

    let doUpVoteByShortkey shortKey =
        let queryResult = getUpVotesForShortKey shortKey
    ...
    let insertByUrl url username =
        let shortKey = string(url.GetHashCode());
        let checkRecordAlreadyExists = getUpVotesForShortKey shortKey
        if checkRecordAlreadyExists |> Seq.length = 0 then
            let upvoteRecord = {PartitionKey = string(url.GetHashCode()); RowKey = username; Url = url; UpVotes = 0 }
            let result = upvoteRecord |> Insert |> inUpVoteTable 
            upvoteRecord
        else
            { PartitionKey = ""; RowKey = ""; Url = ""; UpVotes = 0 }
